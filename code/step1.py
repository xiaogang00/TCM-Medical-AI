...
def binarize_per_slice(image, spacing, intensity_th=-600, sigma=1, area_th=30, eccen_th=0.99, bg_patch_size=10):
    ...
    # image.shape: # of slices, # of image size, # of image size
    ...
    for i in range(image.shape[0]):
        ...
        # All 2D connected components that are smaller than 30mm2 or have eccentricity greater than 0.99 are removed.
...
def all_slice_analysis(bw, spacing, cut_num=0, vol_limit=[0.68, 8.2], area_th=6e3, dist_th=62):
    ...
    # base on real world volume, not need to change anything
    properties = measure.regionprops(label)
    for prop in properties:
...
def step1_python(case_path):
    ...
    while flag == 0 and cut_num < bw.shape[0]:
        bw = np.copy(bw0)
        bw, flag = all_slice_analysis(bw, spacing, cut_num=cut_num, vol_limit=[0.68,7.5], area_th=4e3)
